...
- **Shortcut recorder in-page** — built a `ShortcutRecorder` component that captures keyboard events, supports Windows modifier-only shortcuts (`Ctrl`, `Alt`, `AltGr`), single non-modifier keys, and key combinations, shows a preview, and lets the user confirm or cancel. The Rust `update_shortcut` command validates the format, unregisters all shortcuts, and registers the new one atomically.
...
- `commands/settings.rs::update_shortcut()` parses the shortcut string to validate format, calls `unregister_all()`, persists to DB, then registers the plugin shortcut or enables the modifier-only listener. This ensures the old shortcut is always cleaned up.
- `os/modifier_shortcuts.rs` handles Windows modifier-only shortcuts before the Tauri global-shortcut parser. It watches `Ctrl`, `Alt`, or `AltGr` key state globally and dispatches the same recording state transitions as normal hotkeys.
...
- Modifier-only shortcuts are Windows-only and can conflict with ordinary app gestures such as holding `Ctrl` for copy/paste.
