...
- [x] TASK-201: Rust: `history/reprocess.rs` — `reprocess_session(session_id, language, model_id)` loads the original audio file path from `audio_path` column, invokes whisper sidecar, runs post-processing, updates session record with new results while preserving original `raw_text` in `original_raw_text` column
- [x] TASK-202: Tauri command `reprocess_session` in `commands/history.rs` — wraps `history/reprocess::reprocess_session()`; emits `session-reprocessed` event on completion
- [x] TASK-203: DB migration 5 — add `audio_path TEXT` column to `sessions` table; add `original_raw_text TEXT` column; add `reprocessed_count INTEGER NOT NULL DEFAULT 0` column
- [x] TASK-204: Update `audio/capture.rs` — optionally save recorded audio to app data dir (setting `recording.keep_audio`, default: false); store path in session's `audio_path`; respect max storage setting
  <!-- Audio persistence moved to orchestrator.rs transcribe_and_emit instead of capture.rs for cleaner separation -->
- [x] TASK-205: Frontend: `reprocessSession(sessionId, language?, modelId?)` in `lib/tauri.ts`; add "Reprocess" button to session detail view in History page
- [x] TASK-206: React: Reprocess dialog — modal showing language picker, model picker, and "Reprocess" button; shows progress spinner; displays diff of old vs. new cleaned text on completion
  <!-- Implemented as inline panel in SessionDrawer with "Original" tab for comparing text -->
- [x] TASK-207: Configurable post-processing pipeline — `postprocess/pipeline.rs` reads settings to determine which steps to run: normalization (always on), punctuation (`transcription.auto_punctuation`), capitalization (`transcription.auto_capitalization`), filler removal (`transcription.remove_fillers`), dictionary auto-replace (`dictionary.auto_apply_rules`)
  <!-- pipeline.rs already reads all these settings — no code changes needed -->
- [x] TASK-208: React: Settings page — "Post-Processing" section with toggles for each pipeline step (punctuation, capitalization, filler removal, dictionary auto-apply); each toggle maps to its existing setting key
  <!-- Added dictionary auto-apply toggle in Transcription section + Audio Storage section with retention settings -->
