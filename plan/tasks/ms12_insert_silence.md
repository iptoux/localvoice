...
- [x] TASK-181: Research and upgrade text insertion — replace `enigo 0.1` with `enigo 0.2+` or direct `windows-sys` `SendInput` calls in `os/text_insertion.rs`; measure reliability vs. current approach
- [x] TASK-182: Implement chunked text insertion in `os/text_insertion.rs` — for texts longer than 4000 chars, split into clipboard chunks with sequential paste to avoid target app buffer overflow
- [x] TASK-183: Add configurable insert delay setting `output.insert_delay_ms` (default: 100) in migration 4; expose in settings; used in `text_insertion::insert()` between clipboard write and Ctrl+V
- [x] TASK-184: Fallback logic in `os/text_insertion.rs::insert()` — if paste simulation fails, keep text on clipboard and emit `output-result` with `mode: "clipboard"` and a user-facing message "Text copied — paste manually"
- [x] TASK-185: Detect active window title before insertion via `GetForegroundWindow` + `GetWindowText` (Windows API) in `os/text_insertion.rs`; store in session's `output_target_app` field
- [x] TASK-186: React: Settings page — add "Insert delay" slider (50ms–500ms) in Output section; maps to `output.insert_delay_ms` setting
- [x] TASK-187: DB migration 4 — add settings: `output.insert_delay_ms` (100)
