...
pub fn list_words_for_language(db: &DbConn, language: &str) -> Result<Vec<String>, AppError> {
    let conn = db.lock().unwrap();
    let mut stmt = conn
        .prepare("SELECT word FROM filler_words WHERE language = ?1 ORDER BY length(word) DESC")?;
    let x = stmt
...
pub fn reset_to_defaults(db: &DbConn, language: &str) -> Result<(), AppError> {
    {
        let conn = db.lock().unwrap();
        conn.execute(
            "DELETE FROM filler_words WHERE language = ?1",
            params![language],
        )?;
    }
...
pub fn get_total_count(db: &DbConn) -> Result<i64, AppError> {
    let conn = db.lock().unwrap();
    let count: i64 = conn.query_row("SELECT COUNT(*) FROM filler_removal_log", [], |row| {
        row.get(0)
    })?;
    Ok(count)
}
...
fn seed_defaults(db: &DbConn, language: &str) -> Result<(), AppError> {
    let defaults: &[&str] = match language {
        ...
        "fr" => &[
            "euh", "bah", "ben", "hein", "voilà", "quoi", "genre", "du coup", "en fait", "tu vois",
        ],
        "es" => &[
            "eh", "este", "pues", "bueno", "o sea", "a ver", "sabes", "mira",
        ],
        "it" => &[
            "allora",
            "cioè",
